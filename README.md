...
```
(node build --copy; node build --composer-install; cd site; vendor/bin/phpunit tests/unit/Espo/Modules/PushNotificationManager)
```
...
```
(node build --copy; node build --composer-install; cd site; vendor/bin/phpunit tests/integration/Espo/Modules/PushNotificationManager)
```
...
If your extension contains additional PHP packages, you also need to add `site/custom/Espo/Modules/PushNotificationManager/vendor` to the *scanDirectories* section in *phpstan.neon* config.
...
* `site/custom/`
* `site/client/custom/`
* `site/tests/unit/Espo/Modules/PushNotificationManager`
* `site/tests/integration/Espo/Modules/PushNotificationManager`
...
2. Set *bundled* and *jsTranspiled* to true in `src/files/custom/Espo/Modules/PushNotificationManager/Resources/module.json`.
3. Add `src/files/custom/Espo/Modules/PushNotificationManager/Resources/metadata/app/client.json`
    ```json
    {
        "scriptList": [
            "__APPEND__",
            "client/custom/modules/push-notification-manager/lib/init.js"
        ]
    }
    ```
...
Add the library module path to `src/files/custom/Espo/Modules/PushNotificationManager/Resources/metadata/app/jsLibs.json`

```json
{
    "some-lib": {
        "path": "client/custom/modules/push-notification-manager/lib/some-lib.js"
    }
}
```
